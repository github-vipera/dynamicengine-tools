...
export default class PluginsUIHandler {
  ...
  initUI(){
    var that = this;
    $('#InstalledPluginsMenuItem').bind('click', function(evt){
        that.selectPanel('InstalledPluginsMenuItem');
        //alert('Show installed plugins!');
    });
    $('#InstallMenuItem').bind('click', function(evt){
        that.selectPanel('InstallMenuItem');
