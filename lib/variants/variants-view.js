module.exports = {
    render: function() {
      ...
      <img class="va-white-logo" src="atom://dynamicengine-tools/resources/VA_white.png" style="position:absolute;bottom:4px;right:4px;height:110px;">
        ...
        <div class="section">
          <h1 class="section-heading icon icon-versions">Variants</h1>
          <div class='block'>
              ...
              <div class="panel-body padded">
                ...
                {{#each preference}}
                  <div class="block preference-block">
                    ...
                    <button class='btn icon icon-x inline-block-tight' variant-remove-preference variant-ref="{{../name}}" variant-preference-id="{{name}}"></button>
                  </div>
                {{/each}}
                <!-- end Global -->
                ...
                {{#each platform}}
                  ...
                  {{#each preference}}
                    <div class="block preference-block">
                      ...
                      <button class='btn icon icon-x inline-block-tight' variant-remove-preference variant-ref="{{../../name}}" variant-platform-ref="{{../name}}" variant-preference-id="{{name}}"></button>
                    </div>
                  {{/each}}
                  <div class="line-div"></div>
                {{/each}}
