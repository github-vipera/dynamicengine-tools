      ...
      <div class="plugins-settings-panel" ui-view="main">
        ...
        <div class="section">
          ...
          <div class='block'>
            ...
            <button class='btn btn-primary icon icon-check inline-block-tight'>Save</button>
        ...
        <atom-panel>
          <div class="inset-panel">
              ...
              <div class="panel-body padded">
                <div class="block">
                  <h3>Global settings</h3>
                  <button class='btn icon icon-plus inline-block-tight' variant-add-param vairant-ref="{{name}}"></button>
                </div>
                <!-- Global -->
                {{#each preference}}
                  <div class="block">
                    <label for={{name}}>{{name}}</label>
                    <input class='input-text' type='text' variant-ref="{{../name}}" variant-param-id="{{name}}" name="{{name}}" id="{{../name}}-{{name}}" placeholder='' value="{{value}}">
                  </div>
                {{/each}}
                <!-- end Global -->
                ...
                {{#each platform}}
                  <div class="block">
                    <h3>{{name}} only</h3>
                    <button class='btn icon icon-plus inline-block-tight' variant-add-param-platform variant-ref="{{../name}}" variant-platform-ref="{{name}}"></button>
                  </div>
                  {{#each preference}}
                    <div class="block">
                      <label for={{name}}>{{name}}</label>
                      <input class='input-text' type='text' name="{{name}}" variant-param-platform-id="{{name}}" id="{{../../name}}-{{../name}}-{{name}}" placeholder='' value="{{value}}">
                    </div>
                  {{/each}}
                  <div class="line-div"></div>
                {{/each}}
