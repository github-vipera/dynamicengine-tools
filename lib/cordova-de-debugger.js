...
import PluginsUIHandler from './plugins/PluginsUIHandler.js';
...
export default {
  ...
  openPluginsManager(){
    Logger.debug("openPluginsManager");
    var pluginsView = require("./plugins/plugins-with-menu");
    var runSettingsViewProm = Pane({
        element: 'div',
        title: 'Pluging management',
        split: 'center',
        activatePane: true
      },  (err, div) => {
        if (err) throw err
        this.pluginsViewDiv = div;
        div.innerHTML = pluginsView.render();
        if(!this.pluginManagerUIHandler){
          var executor = new CommandExecutor(Logger);
          this.pluginManagerUIHandler = new PluginsUIHandler(div, executor, this.compositeServer);
        }
        this.pluginManagerUIHandler.init();
      },  () => {
          console.info("openPluginsManager closed");
          //this.runSettingsDiv=undefined;
          //this.runSettingsUIHandler.destroyDiv();
      });
      runSettingsViewProm.then((value) => {
