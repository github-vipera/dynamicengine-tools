...
var LoaderServiceBridge = require('../LoaderServiceBridge');
module.exports = function(logger, nativeStorage, deStorage, deUtilities, lodash, implInfo) {
    ...
    //window._ = lodash;
    var serviceBridge=new LoaderServiceBridge();
    serviceBridge.addService("NativeStorage",nativeStorage);
    serviceBridge.addService("DEStorage",deStorage);
    serviceBridge.addService("Utilities",deUtilities);
    ...
    if(implInfo.libraryLoader){
      ...
      libraryLoader(reload,serviceBridge);
    }else{
