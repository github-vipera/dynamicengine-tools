...
export default class RunSettingsUIHandler {
  ...
  addPlatformsToSelect(items){
    ...
    $.each(items, function (i, item) {
      var value=CordovaUtils.getPlatformValue(item);
  ...
  bindJS(){
    ...
    * button build
    */
    $("#derunsettings-btn-build").on('click',() => {
    ...
    * button run
    */
    $("#derunsettings-btn-run").on('click',() => {
    ...
    * button stop
    */
    $("#derunsettings-btn-stop").on('click',() => {
    ...
    * button clear
    */
    $("#derunsettings-btn-clear").on('click',() => {
    ...
    */
    $('#derunsettings-platform-select').val(null)
    $('#derunsettings-platform-select').on('change',() => {
      var platform = $('#derunsettings-platform-select').val();
      if(platform){
    ...
    */
    $('#derunsettings-device-select').val(null);
    $('#derunsettings-device-select').on('change',() => {
      var target = $('#derunsettings-device-select').val();
      if(target){
  ...
  bindJSRemoteServerSection(){
    $('#derunsettings-txt-serverMockImpl').on("drop", function(event){
    ...
    $('#derunsettings-txt-localdatabase').on("drop", function(event){
       event.preventDefault();
       var filePath = event.originalEvent.dataTransfer.getData("initialpath");
       if (filePath){
    ...
    $('#derunsettings-txt-libraryLoader').on("drop", function(event){
       event.preventDefault();
       var filePath = event.originalEvent.dataTransfer.getData("initialpath");
       if (filePath){
    ...
    $('#derunsettings-server-btn-run').on('click',() =>{
      this.startPlatformServer();
    });

    $('#derunsettings-server-btn-stop').on('click',() =>{
      this.setStatusIndicator(false);
      this.platformServer.stopServer();
    });

    $('#derunsettings-server-btn-build').on('click',() =>{
        //TODO
    });

    $('#derunsettings-server-btn-liveReload').on('click',() =>{
      this.doLiveReload();
    });
  }
  ...
  launchBuildOnPlatform(platform){
    this.setWaitTo(true);
    this.cmdExecutor.execBuildWithSpawn(platform).then((res) => {
  ...
  launchRunOnPlatform(platform,target){
    this.setWaitTo(true);
    this.cmdExecutor.execRunWithSpawn(platform,target).then((res) => {
  ...
  createConfig(){
    return{
      ...
      localdatabase : $('#derunsettings-txt-localdatabase').val(),
  ...
  startPlatformServer(){
    if(!this.platformServer.isRunning()){
      ...
      this.platformServer.startServer(config);
    }
    this.setStatusIndicator(true);
  }
  setStatusIndicator(vale){
    if(vale){
